...
export function InvoiceReviewPage({ sourceFileId }: { sourceFileId: string }) {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
    ...
    {data.header.extractionStatus === 'failed' && <Alert className="mb-5"><AlertCircle /><AlertTitle>Automatic extraction unavailable</AlertTitle><AlertDescription>The invoice is still ready for manual review. Every field remains editable.</AlertDescription></Alert>}
    {data.header.extractionStatus === 'not_started' && <Alert className="mb-5"><FileText /><AlertTitle>Ready for review</AlertTitle><AlertDescription>Automatic extraction is not configured. Complete or edit the invoice details manually.</AlertDescription></Alert>}
