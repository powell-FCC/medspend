...
export function InvoiceReviewPage({ sourceFileId }: { sourceFileId: string }) {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
    ...
    {data.header.structuredExtractionState === 'STRUCTURED_SUCCESS' && <Alert className="mb-5 border-emerald-200 bg-emerald-50"><CheckCircle2 className="text-emerald-700" /><AlertTitle>Invoice details extracted.</AlertTitle><AlertDescription>Review the pre-filled details and line items, then correct anything that does not match the PDF.</AlertDescription></Alert>}
    {data.header.structuredExtractionState === 'STRUCTURED_PARTIAL' && <Alert className="mb-5 border-amber-200 bg-amber-50/50"><AlertCircle className="text-amber-700" /><AlertTitle>Invoice partially extracted.</AlertTitle><AlertDescription>Some invoice details could not be identified reliably. Review the suggested fields and add anything missing.</AlertDescription></Alert>}
    {(data.header.structuredExtractionState === 'MANUAL_REVIEW_REQUIRED' || data.header.structuredExtractionState === 'STRUCTURED_FAILED') && <Alert className="mb-5"><FileText /><AlertTitle>Automatic invoice structuring needs manual review.</AlertTitle><AlertDescription>Automatic structuring was unable to identify enough information reliably. Manual review is fully available.</AlertDescription></Alert>}
    {data.header.documentTextStatus === 'success' && data.header.extractionStatus !== 'succeeded' && !data.header.structuredExtractionState && <Alert className="mb-5"><FileText /><AlertTitle>Document text extracted successfully.</AlertTitle><AlertDescription>Invoice structure could not be determined reliably. Manual review remains fully available.</AlertDescription></Alert>}
